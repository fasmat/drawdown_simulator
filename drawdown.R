...
plot_drawdown <- function(payouts) {
    ...
    plot(seq_along(payouts_median), payouts_median,
        type = "l", col = "blue",
        xlab = "Year", ylab = "Median Payout", main = "Median Payout Over Time",
        ylim = c(0, max(payouts_median) * 1.1)
    )
    lines(seq_along(payouts_q05), payouts_q05, col = "darkred", lty = 2)
    lines(seq_along(payouts_q10), payouts_q10, col = "red", lty = 2)
    lines(seq_along(payouts_q25), payouts_q25, col = "orange", lty = 2)
    legend("topright",
        legend = c("Median", "5th Percentile", "10th Percentile", "25th Percentile"),
    ...
    text(
        x = length(payouts_median) * 0.2, y = max(payouts_median) * 0.9,
        labels = paste0(
